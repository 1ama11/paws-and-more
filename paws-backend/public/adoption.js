// adoption.js - Updated to work with backend
    ...
    if (adoptForm) {
        adoptForm.addEventListener('submit', async (e) => {
            ...
            if (isValid) {
                ...
                const otherPets = document.querySelector('input[name="otherPets"]:checked').value;
                const petName = document.getElementById('pName').value;
    ...
    if (detailTitle && petName) {
        fetch(`/api/pet/${petName}`)
            ...
            .catch(err => {
                ...
                if (detailTitle) {
                    detailTitle.innerText = "Pet not found";
                }
            });
    }
});